...
4. **Consider (do not assume) updates to these files.** For each, decide whether the notes justify a change and skip the ones that do not:
   - The active course's `KEY_TAKEAWAYS.md` — for the Kodree course this is `courses/kodree-claude-code/KEY_TAKEAWAYS.md`. Each course keeps its own `KEY_TAKEAWAYS.md` inside its own `courses/<course-folder>/` directory; there is no repo-root takeaways file.
   - `playbooks/`
   - `GLOSSARY.md`
   - `LEARNING_HUB.md`
   - `SKILLS_MATRIX.md`
